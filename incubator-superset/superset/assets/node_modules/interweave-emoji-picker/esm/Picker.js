...
import { GROUPS, GROUP_KEY_COMMONLY_USED, GROUP_KEY_SMILEYS_PEOPLE, GROUP_KEY_SEARCH_RESULTS, GROUP_KEY_NONE, SKIN_TONES, SKIN_KEY_NONE, KEY_COMMONLY_USED, KEY_SKIN_TONE, COMMON_MODE_RECENT, COMMON_MODE_FREQUENT, CONTEXT_CLASSNAMES, CONTEXT_MESSAGES, SEARCH_THROTTLE } from './constants';
var SKIN_MODIFIER_PATTERN = /1F3FB|1F3FC|1F3FD|1F3FE|1F3FF/g;
export var Picker = function (_React$PureComponent) {
  _inheritsLoose(Picker, _React$PureComponent);
  ...
  function Picker(props) {
    ...
    _defineProperty(_assertThisInitialized(_this), "blacklist", void 0);

    _defineProperty(_assertThisInitialized(_this), "whitelist", void 0);

    _defineProperty(_assertThisInitialized(_this), "handleClearCommonEmoji", function () {
    ...
    _this.blacklist = _this.generateBlackWhiteMap(blacklist);
    _this.whitelist = _this.generateBlackWhiteMap(whitelist);
    var _searchQuery = '';
  ...
  _proto.filterOrSearch = function filterOrSearch(emoji, searchQuery) {
    ...
    if (blacklist.length > 0 && this.blacklist[emoji.hexcode] || whitelist.length > 0 && !this.whitelist[emoji.hexcode]) {
      return false;
    }
  ...
  _proto.generateBlackWhiteMap = function generateBlackWhiteMap(list) {
    var map = {};
    list.forEach(function (hexcode) {
      if ("production" !== process.env.NODE_ENV) {
        if (hexcode.match(SKIN_MODIFIER_PATTERN)) {
          console.warn("Hexcode with a skin modifier has been detected: " + hexcode, 'Emojis without skin modifiers are required for blacklist/whitelist.');
        }
      }
  ...
  _proto.getSkinnedEmoji = function getSkinnedEmoji(emoji, skinTone) {
    ...
    var skinnedEmoji = (emoji.skins || []).find(function (skin) {
      return !!skin.tone && skin.tone === toneIndex;
    });
  ...
  _proto.setUpdatedState = function setUpdatedState(nextState) {
    var _this3 = this;
  ...
  _proto.render = function render() {
    var _ref3 = this.props,
        ...
        emojiLargeSize = _ref3.emojiLargeSize,
        emojiPadding = _ref3.emojiPadding,
        emojiPath = _ref3.emojiPath,
        emojiSize = _ref3.emojiSize,
        emojiSource = _ref3.emojiSource,
...
_defineProperty(Picker, "propTypes", {
  ...
  messages: _pt.objectOf(_pt.string)
});
